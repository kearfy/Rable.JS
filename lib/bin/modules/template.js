...
class Template {
    ...
    constructor(master) {
        ...
        const finish = () => {
            if (master.config.template !== undefined && master.config.template !== false && master.config.template !== null && master.config.template !== '') {
                ...
                Object.keys(build.elements).forEach(k => elementChanged[k] = false);
            ...
            finished = true;
            scheduledElementTasks.forEach(task => {
                if (build !== null) {
                    build.elements[task.element] = task.value;
                    elementChanged[task.element] = true;
                }
            })
        }
    }
    ...
    getElement(element) {
        if (build === null) {
            ...
            if (elementChanged[element] !== undefined && !elementChanged[element]) {
                return build.root + build.elements[element];
            } else {
                return build.elements[element];
            }
        }
    }
