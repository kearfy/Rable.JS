 ...
 * @property {Function} info.get Get the value of a property.
 * @property {Function} info.set Set the value of a property.
...
class Rable {
    ...
    registerRoute(input) {
        class Route {
            constructor(input) {
                ...
                if (input.method == 'static' && typeof input.action != 'string') console.error('The action of a static route must be a path to a directory');
            }
    ...
    matchRoute(url, method = false) {
        ...
        Object.keys(routes).forEach(route => {
            if (finalRoute === null) {
                route = routes[route];
                if (!method ? true : (route.method == method || route.method == 'static')) {
                    var result = this.matchUrl(route.path, url, route.method == 'static');
            ...
            explodedGiven.forEach((part, index) => {
                if (explodedOriginal[index] !== part) {
                    if (explodedOriginal[index].charAt(0) === ':') {
                        ...
                        params[param] = part;
                    } else if (explodedOriginal[index] == '~' && index == explodedOriginal.length - 1) {
